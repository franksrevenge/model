...
utils.mixin(model, new (function () {
  ...
  this.registerDefinition = function (name, ModelDefinition) {
    ...
    ModelDefinition.prototype = utils.mixin( ModelDefinition.prototype, new model.ModelDefinitionBase(name) );
  ...
  this.filterParams = function (item, passedParams, opts) {
      ...
      for (var p in properties) {
        if (passedParams[p] && properties[p].options.filter) {
          passedParams[p] = filters.processFilters(properties[p].options.filter, passedParams[p]);
        }
      }
  };
