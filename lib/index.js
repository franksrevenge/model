...
utils.mixin(model, new (function () {
  ...
  this.validateAndUpdateFromParams = function (item, passedParams, opts) {
    ...
    this.filterParams(item, passedParams, opts);
  ...
  this.filterParams = function (item, passedParams, opts) {
      var name = item.type
        , type = model.descriptionRegistry[name]
        , properties = type.properties
        , filteredValue;
      ...
      for (var p in properties) {
        if (passedParams[p] && properties[p].options.filter) {
          if (properties[p].options.filter instanceof Array) {
            filteredValue = passedParams[p];

            for (var i = 0; i < properties[p].options.filter.length; i++) {
              filteredValue = this.processFilters(filteredValue, properties[p].options.filter[i], item);
            }

            passedParams[p] = filteredValue;
          }
          else
          {
            passedParams[p] = this.processFilters(passedParams[p], properties[p].options.filter, item);
          }
        }
      }
  };

  this.processFilters = function (paramValue, filter, item) {
    ...
    switch (typeof filter) {
      ...
      case 'object':
        if (filter instanceof RegExp) {
          return paramValueStr.match(filter, '');
        }

        if (filter.type == 'replace' ) {
          return paramValueStr.match(filter.search, filter.replacement, filter.flags);
        }
        break;
  ...
  this.validateProperty = function (prop, params, opts) {
    ...
    // validation routines if that's the case.
